...
import { Level2Snapshot, Market, Level2Update } from './app.interfaces';
...
export class OrderbookManager implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    this.eventBus.on('ccxws.l2snapshot', (x) => this.bufferOrderbookUpdate(x.market.id, x.l2snapshot));
    this.eventBus.on('ccxws.l2update', (x) => this.bufferOrderbookUpdate(x.market.id, x.l2update));
    this.setBufferFlushInterval(1000);
  }
  ...
  bufferOrderbookUpdate(symbol: string, data: Level2Snapshot | Level2Update): void {
  ...
  setBufferFlushInterval(ms: number) {
